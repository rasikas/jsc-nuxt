...
export default {
  ...
  async mounted() {
    ...
    if (this.onCreatedOpenForm && this.record.id /* Not Needed? && !this.parentId */) {
      // nested CRUD, when coming back to a parent open a form
    ...
    if (this.formAutoData) {
      // type to field // TODEPRECATE
  ...
  data() {
    return {
      ...
      inlineReload: {
        // set to false for services where record read is chargeable, e.g. Google Firestore (use listeners instead)
        create: true,
        update: true,
        delete: true
      },
      ...
      attrs: {
        ...
        snackbar: {
          // v-snackbar Component - null means no snack bar
          bottom: true,
          timeout: 6000
        },
        container: {
          // v-container Component
          fluid: true,
        ...
        table: {
          ...
          style: {
            // this may need to be changed once Vuetify version 2.0 is out
        ...
        'v-progress-linear': {
          // v-progress-linear, can also be v-progress-circular
          class: 'ma-0'
        },
  ...
  computed: {
    ...
    pageData() {
      return this.$store.getters[this.storeName + '/pageData']
    }
  ...
  methods: {
    ...
    async updateRecord(payload) {
      ...
      this.setSnackBar(res)
      if (typeof res === 'object') return res.ok
      // TODEPRECATE this check
    ...
    isEditing(row) {
      ...
      if (row) {
        ...
        // check if entire table is editing
        return this.editing
      }
    },
    setEditing(row, item) {
    ...
    async inlineClose(item, field, row, col) {
      if (!field || item[field] !== this.inlineValue) {
        // field undefined means saveRow button clicked
        if (field && this.saveRow) {
          // cell changed
  ...
  <v-container v-if="ready" v-bind="attrs.container">
    <slot name="table-toolbar" :vcx="_self">
      <v-toolbar v-bind="attrs.toolbar">
        ...
        <v-toolbar-title>
          <v-btn v-if="parentId && showGoBack" v-bind="attrs.button" @click.stop="goBack" :disabled="loading">
            <v-icon>{{ buttons.back.icon }}</v-icon>
            <span>{{ buttons.back.label }}</span>
          </v-btn>
          {{ showTitle | capitalize }} {{ doPage ? '' : ` (${records.length})` }}
        </v-toolbar-title>
        <v-spacer></v-spacer>
        <v-btn v-if="showFilterButton || hasFilterSlot" v-bind="attrs.button" @click="expandFilter = !expandFilter" :disabled="!hasFilterData">
          <v-icon>{{ expandFilter ? buttons.filter.icon2 : buttons.filter.icon }}</v-icon>
          <span>{{ buttons.filter.label }}</span>
        </v-btn>
        <v-btn v-bind="attrs.button" @click="submitFilter" :disabled="!validFilter || loading">
          <v-icon>{{ buttons.reload.icon }}</v-icon>
          <span>{{ buttons.reload.label }}</span>
        </v-btn>
        <v-btn v-if="canCreate" v-bind="attrs.button" @click.stop="addrowCreate ? inlineCreate() : crudFormOpen(null)" :disabled="loading">
          <v-icon>{{ buttons.create.icon }}</v-icon>
          <span>{{ buttons.create.label }}</span>
        </v-btn>
        <v-btn v-if="crudOps.export" v-bind="attrs.button" @click.stop.prevent="exportBtnClick" :disabled="loading">
          <v-icon>{{ buttons.export.icon }}</v-icon>
          <span>{{ buttons.export.label }}</span>
        </v-btn>
      </v-toolbar>
    </slot>
            ...
            <v-flex v-for="(filter, index) in filterData" :key="index" :sm6="filter.halfSize" xs12>
              <component :is="filter.field" v-model="filter.value" v-bind="filter.attrs">
                <template v-if="filter.field === 'v-btn-toggle'">
                  <component :is="'v-btn'" v-for="(value, key, index) in filter.group.items" :key="index" :value="key" v-bind="filter.group.attrs">{{
                    value
                  }}</component>
                </template>
                <template v-else-if="filter.field === 'v-radio-group'">
                  <component
                    :is="'v-radio'"
                    v-for="(value, key, index) in filter.group.items"
                    :key="index"
                    :value="key"
                    :label="value"
                    v-bind="filter.group.attrs"
                  ></component>
                </template>
              </component>
            </v-flex>
      ...
      >
          ...
          <tr :ref="`edit-${props.index}`" @click.stop="rowClicked(props.item, $event, props.index)">
            <td :key="header.value" v-for="(header, index) in headers" :class="header['cell-class'] ? header['cell-class'] : header.class">
              <span v-if="header.value === ''">
                <v-icon v-if="canUpdate && !saveRow" v-bind="attrs['action-icon']" @click.stop="crudFormOpen(props.item.id)" :disabled="loading"
                  >edit</v-icon
                >
                <v-icon v-if="canDelete" v-bind="attrs['action-icon']" @click.stop="inlineDelete(props.item.id)" :disabled="loading">delete</v-icon>
                <v-icon
                  v-if="crudOps.update && saveRow"
                  v-bind="attrs['action-icon']"
                  @click.stop="inlineUpdate(props.item, null, props.index, index)"
                  :disabled="loading"
                  >save</v-icon
                >
              </span>
              ...
              <v-edit-dialog
                v-else-if="
                  inline[header.value].field === 'v-date-picker' ||
                    inline[header.value].field === 'v-time-picker' ||
                    inline[header.value].field === 'v-textarea'
                "
              ...
              >
                ...
                <component
                  ...
                  @blur="
                    inline[header.value].field === 'v-textarea'
                      ? inlineUpdate(props.item, header.value, props.index, index)
                      : inlineClose(props.item, header.value, props.index, index)
                  "
              ...
              >
                <template v-if="inline[header.value].field === 'v-btn-toggle'">
                  <component
                    :is="'v-btn'"
                    v-for="(value, key, index) in inline[header.value].group.items"
                    :key="index"
                    :value="key"
                    :label="value"
                    v-bind="inline[header.value].group.attrs"
                  ></component>
                </template>
        ...
        <template slot="no-data">
          <v-flex class="text-xs-center">
            <v-alert :value="true" v-bind="attrs.alert">
      ...
      <v-dialog v-model="crudFormFlag" v-bind="attrs.dialog">
        <v-card>
          <slot name="form-toolbar" :vcx="_self">
            <v-toolbar v-bind="attrs.toolbar">
              <v-toolbar-title>
                <v-btn v-bind="attrs.button" @click.native="closeCrudForm" :disabled="loading">
                  <v-icon>{{ buttons.close.icon }}</v-icon>
                  <span>{{ buttons.close.label }}</span>
                </v-btn>
                {{ showTitle | capitalize }}
              </v-toolbar-title>
              <v-spacer></v-spacer>
              <v-toolbar-items>
                <v-btn v-bind="attrs.button" v-if="canDelete && record.id" @click.native="crudFormDelete" :disabled="loading">
                  <v-icon>{{ buttons.delete.icon }}</v-icon>
                  <span>{{ buttons.delete.label }}</span>
                </v-btn>
                <v-btn
                  v-bind="attrs.button"
                  v-if="(canUpdate && record.id) || (canCreate && !record.id)"
                  :disabled="!validForm || loading"
                  @click.native="crudFormSave"
                >
                  <v-icon>{{ buttons.update.icon }}</v-icon>
                  <span>{{ buttons.update.label }}</span>
                </v-btn>
              </v-toolbar-items>
            </v-toolbar>
          </slot>
          <component
            :is="attrs['v-progress-circular'] ? 'v-progress-circular' : 'v-progress-linear'"
            :indeterminate="loading"
            v-bind="attrs['v-progress-circular'] ? attrs['v-progress-circular'] : attrs['v-progress-linear']"
          ></component>
                ...
                <v-flex v-for="(form, objKey, index) in formAutoData" :key="index" :sm6="form.halfSize" xs12>
                  ...
                  <component v-else-if="record[objKey] !== undefined" :is="form.field" v-model="record[objKey]" v-bind="form.attrs">
                    <template v-if="form.field === 'v-btn-toggle'">
                      <component :is="'v-btn'" v-for="(value, key, index) in form.group.items" :key="index" :value="key" v-bind="form.group.attrs">{{
                        value
                      }}</component>
                    </template>
                    <template v-else-if="form.field === 'v-radio-group'">
                      <component
                        :is="'v-radio'"
                        v-for="(value, key, index) in form.group.items"
                        :key="index"
                        :value="key"
                        :label="value"
                        v-bind="form.group.attrs"
                      ></component>
                    </template>
                  </component>
                </v-flex>
    ...
    <v-snackbar v-if="attrs.snackbar" v-model="snackbar" v-bind="attrs.snackbar">
      {{ snackbarText }}
      <v-btn fab flat @click="snackbar = false">
        <v-icon>close</v-icon>
      </v-btn>
    </v-snackbar>
  </v-container>
